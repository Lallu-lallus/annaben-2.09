...
IMDB_TEMPLATE = environ.get("IMDB_TEMPLATE","<b>Query: {query}🏷️ <b>Title</b>: <a href={url}>{title}</a>\n📜 Type: <code>{kind}</code>\n📆 Release Date: <code>{release_date}</code>\n🗓️ Year: <code>{year}</code>\n🎭 Genres: <code>{genres}</code>\n⭐ Rating: {rating} /10\n🗣️ Languages: <code>{languages}</code>\n👥 Cast: <code>{cast}</code>\n🎬 Director: <code>{director}</code>\n⏰ Runtime: {runtime} minutes\n🇮🇳 Countries: <code>{countries}</code>\n✍️ Story Line: <code>{plot}</code>")
